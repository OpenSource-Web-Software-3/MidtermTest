...
public class getItemList_to_SubCateAction extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if (request.getParameter("main_cateOfcategorySkin") != null || request.getParameter("main_cateOfcategorySkin").equals("")) {
	...
	public String getJSON(String[] sub_cate, String[] main_cate, String main_cateOfcategorySkin, String pageTitle,
		...
		if (sub_cate.length == 0) { // 체크를 모두 해제했을 경우 모든 리스트 보여주기 main_cate 가져와야함
			if (main_cateOfcategorySkin != null) { // category-skin.jsp에서의 호출일 경우
				...
				for (int i = 0; i < shopCartList.size(); i++) itemList.add(new ItemDAO().getItem(shopCartList.get(i).getItemCode()));
			} else {
				System.out.println("여기는 오류");
			}
		...
		if (pageTitle.equals("Purchase List")) result.append("], \"flag\":\"Purchase List\"}");
		else if (pageTitle.equals("Shop Cart")) result.append("], \"flag\":\"Shop Cart\"}");
		else  result.append("], \"flag\":\"category-skin\"}"); //category-skin.jsp
